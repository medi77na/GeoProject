...
from backend.services.kpi_calculator import KPIResult
...
def _build_simulation_from_kpis(kpis: RecommendationKpis, scenario_name: str) -> SimulationResponse:
    time = [0, 8 * 60, 18 * 60]
    ...
    for zone in kpis.zones:
        ...
        traffic_series = [_clamp_unit(zone.congestion)] * len(time)
        pollution[zone.name] = pollution_series[: len(time)]
        traffic[zone.name] = traffic_series

    if not pollution:
        pollution["Overall"] = [float(kpis.pm25_avg), float(kpis.pm25_max), float(kpis.pm25_avg)]
        traffic["Overall"] = [_clamp_unit(kpis.congestion_index)] * len(time)
...
def _build_kpi_result(kpis: RecommendationKpis, scenario_name: str) -> KPIResult:
    ...
    for zone in kpis.zones:
        ...
        congestion = _clamp_unit(zone.congestion)
    ...
    if not kpis.zones:
        ...
        congestion_index["Overall"] = _clamp_unit(kpis.congestion_index)
        traffic_index["Overall"] = _clamp_unit(kpis.congestion_index)
...
def build_recommendation_response(
    ...
    if (
        ...
        try:
            ...
            logger.exception("AI commentary generation failed; skipping optional commentary.")
