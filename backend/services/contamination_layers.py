...
from backend.models.simulation import PointData, ZoneData
from backend.services.map_layers import get_zones_geojson
...
def _zone_centers() -> Dict[str, PointLike]:
    geojson = get_zones_geojson()
...
def build_zone_summaries(
    ...
    traffic_averages = [
        _average(traffic.get(zone, [])) for zone in zones_list if traffic.get(zone)
    ]
    ...
    for zone in zones_list:
        ...
        avg_pm25 = _average(pollution_values)
        avg_traffic = _average(traffic_values)

        traffic_rel = (
            _clamp_unit(avg_traffic / max_traffic_avg) if max_traffic_avg > 0 else 0.0
        )
        summaries.append(
...
def build_point_samples(
    ...
    for zone in zones:
        ...
        samples_to_generate = max(3, min(len(offsets), total_steps, max_points_per_zone))
        ...
        for idx in range(samples_to_generate):
            ...
            # Small deterministic variation to avoid identical values across points.
...
def build_heatmap(points: Sequence[PointData]) -> List[List[float]]:
    ...
    for point in points:
        intensity = _clamp_unit(point.pm25 / max_pm)
