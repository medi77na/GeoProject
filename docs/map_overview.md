...
- `POST /api/v1/simulate` keeps its existing contract. The map consumes the latest traffic and pollution values returned per zone (last timestep).
...
- `leaflet_layers.js` – composable Leaflet layers (OSM base tiles, polygons with tooltips/popups, traffic markers).
- `index.js` – `<UrbanMap />` component that fetches `/api/v1/map/zones`, merges the latest simulation metrics, and renders the map + legend.
